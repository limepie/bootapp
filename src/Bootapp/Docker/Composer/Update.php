...
class Update extends \Bootapp\Command
{
    protected function configure()
    {
        $this
            ->setName('composer:update')
            ->setDescription('Updates your dependencies to the latest version according to composer.json');
    }
    ...
    public function process()
    {
        ...
        // compose install
        {
            ...
            if ($container) {
                $message = $this->command('docker exec -i $(docker ps -f name='.$projectName.'_php -q) sh -c  "cd /var/www/ && composer update --prefer-dist -vvv --profile"');
